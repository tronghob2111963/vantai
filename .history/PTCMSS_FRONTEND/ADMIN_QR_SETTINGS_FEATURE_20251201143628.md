...
1. Admin đăng nhập và vào menu "Cấu hình hệ thống"
2. Scroll xuống phần "Cấu hình thanh toán QR"
3. Form load giá trị hiện tại từ API
4. Admin chỉnh sửa các trường (STK, tên TK, mã NH, mã mô tả)
5. Click "Lưu cấu hình"
6. Frontend gửi PUT request → Backend validate → Save DB → Clear cache
...
2. Vào `/admin/settings` (Cấu hình hệ thống)
3. Scroll xuống phần "Cấu hình thanh toán QR"
4. **Expected**: Form hiển thị giá trị hiện tại (từ DB hoặc config)

### Test Case 2: Update Settings
1. Login admin → vào `/admin/settings`
2. Scroll xuống phần QR settings
3. Thay đổi STK: `070122047995` → `123456789012`
4. Thay đổi tên: `NGUYEN VAN THUAN` → `TRAN THI B`
5. Click "Lưu cấu hình"
6. **Expected**: Success message, form reset với giá trị mới
...
2. Try vào `/admin/settings`
3. **Expected**: Redirect hoặc 403 Forbidden (toàn bộ trang settings, bao gồm QR)
...
1. Admin vào `/admin/settings` (Cấu hình hệ thống)
2. Scroll xuống phần "Cấu hình thanh toán QR"
3. Form sẽ hiển thị giá trị từ `application.yml` (fallback)
4. Click "Lưu cấu hình" → giá trị được lưu vào DB
5. Từ lần sau, hệ thống ưu tiên đọc từ DB
