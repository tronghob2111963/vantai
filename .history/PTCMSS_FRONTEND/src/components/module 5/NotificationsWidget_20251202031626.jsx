            ...
            if (driverId && startDate) {
                try {
                    ...
                    if (conflicts.length > 0) {
                        const tripsList = conflicts.map((t, i) => 
                            `  ${i + 1}. Chuyến #${t.tripId || t.trip_id || t.id} - ${new Date(t.startTime || t.start_time).toLocaleDateString('vi-VN')}`
                        ).join('\n');
                        
                        const confirmMsg = `⚠️ CẢNH BÁO: Tài xế có ${conflicts.length} chuyến trong thời gian nghỉ.\n\n` +
                            `Các chuyến xung đột:\n${tripsList}\n\n` +
                            `✅ Nếu bạn chấp nhận, hệ thống sẽ TỰ ĐỘNG:\n` +
                            `   • Xóa tài xế khỏi ${conflicts.length} chuyến trên\n` +
