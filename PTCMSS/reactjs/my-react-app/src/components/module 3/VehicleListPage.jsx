...
export default function VehicleListPage() {
    ...
    const filteredSorted = React.useMemo(() => {
        ...
        const afterFilter = vehicles.filter((v) => {
            ...
            if (categoryFilter && v.category_id !== categoryFilter)
                return false;
            if (statusFilter && v.status !== statusFilter) return false;
            if (
                q &&
                !String(v.license_plate).toLowerCase().includes(q)
            )
                return false;
            return true;
        });
    ...
    const handleCreateSubmit = (payload) => {
        const newId =
            Math.max(0, ...vehicles.map((v) => v.id)) + 1;

        const cat = MOCK_CATEGORIES.find(
            (c) => c.id === payload.category_id
        );
        const br = MOCK_BRANCHES.find(
            (b) => b.id === payload.branch_id
        );
        ...
        const newVehicle = {
            ...
            seats: cat ? cat.seats : undefined,
            branch_id: payload.branch_id,
            branch_name: br ? br.name : "—",
            status: payload.status || "AVAILABLE",
            reg_due_date: payload.reg_due_date || "",
            ins_due_date: payload.ins_due_date || "",
            model: payload.model || "",
            year: payload.year || "",
        };

        setVehicles((prev) => [newVehicle, ...prev]);
        push("Đã thêm xe mới " + payload.license_plate, "success");
    };
        ...
        setVehicles((prev) =>
            prev.map((v) => {
                ...
                return {
                    ...v,
                    branch_id: payload.branch_id,
                    branch_name: br ? br.name : v.branch_name,
                    category_id: payload.category_id,
                    category_name: cat ? cat.name : v.category_name,
    ...
    const handleRefresh = () => {
        ...
        setTimeout(() => {
            setVehicles((old) => [...old]); // mock refetch
